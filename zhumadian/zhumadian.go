...
var zhumadianToken string
var zhumadianDevices []xphapi.Device
...
func Start() {
	...
	c := cron.New()
	_ = c.AddFunc("0 0 0/12 * * *", zhumadianUpdateToken)
	_ = c.AddFunc("0 0 0/1 * * *", zhumadianUpdateDevices)
	_ = c.AddFunc("0 0/10 * * * *", zhumadianSendData)
	c.Start()
	defer c.Stop()
	select {}
}

func zhumadianUpdateToken() {
	zhumadianToken = xphapi.GetToken("zhumadian", "123456")
}

func zhumadianUpdateDevices() {
	zhumadianDevices = xphapi.GetDevices("zhumadian", zhumadianToken)
}

func zhumadianSendData() {
	for _, item := range zhumadianDevices {
		...
		if len(dataEntity.Entity) > 0 {
			...
			if datatime.After(time.Now().Add(-time.Hour)) {
				...
				utils.Invoke("http://112.53.105.18:7001/sanitate/services/SaveYCJCService?", content)
			} else {
