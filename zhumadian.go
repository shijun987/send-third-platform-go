...
var zhumadianDevices []Device

// ZhumadianStart zhumadian
func ZhumadianStart() {
...
func zhumadianUpdateToken() {
	zhumadianToken = GetToken("zhumadian", "123456")
}

func zhumadianUpdateDevices() {
	zhumadianDevices = GetDevices("zhumadian", zhumadianToken)
}
...
func zhumadianSendData() {
	for _, item := range zhumadianDevices {
		...
		if len(dataEntity.Entity) > 0 {
			...
			Invoke("http://112.53.105.18:7001/sanitate/services/SaveYCJCService?", content)
		} else {
