...
func Start() {
	...
	c := cron.New()
	_ = c.AddFunc("0 0 0/12 * * *", nanyangUpdateToken)
	_ = c.AddFunc("0 0 0/1 * * *", nanyangUpdateDevices)
	_ = c.AddFunc("0 0/10 * * * *", nanyangSendData)
	c.Start()
	defer c.Stop()
	select {}
}
...
func nanyangUpdateToken() {
	nanyangToken = xphapi.GetToken("nanyang", "123456")
}

func nanyangUpdateDevices() {
	nanyangDevices = xphapi.GetDevices("nanyang", nanyangToken)
}
